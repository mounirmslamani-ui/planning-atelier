      ...
      <fieldset disabled={lock.locked} className="border-0 p-0 m-0 space-y-5">
        ...
        <section className="rounded-lg border bg-card">
                      ...
                      return (
                        <tr key={`${step.id}-${it.id}`} className="border-b last:border-0">
                          ...
                          <td className="p-2 min-w-36">
                            <MoneyInput value={it.costPrice} onValueChange={v => patchItem(step.id, it.id, { costPrice: v })} />
                          </td>
          ...
          <div className="px-3 py-2 border-t text-xs flex justify-between">
            ...
            <span className="font-semibold">{formatDZD(breakdown.materialsSaleTotal)}</span>
          </div>
        </section>
        ...
        <section className="rounded-lg border bg-card">
                ...
                {draftSteps.filter(s => s.subcontractorId).map(step => {
                  ...
                  return (
                    <tr key={step.id} className="border-b last:border-0">
                      ...
                      <td className="p-2 min-w-36">
                        <MoneyInput value={step.subcontractingCost} onValueChange={v => patchStep(step.id, { subcontractingCost: v })} />
                      </td>
                      <td className="p-2 min-w-28">
                        <SearchableSelect
          ...
          <div className="px-3 py-2 border-t text-xs flex justify-between">
            ...
            <span className="font-semibold">{formatDZD(breakdown.subcontractingSaleTotal)}</span>
          </div>
        </section>
        ...
        <section className="rounded-lg border bg-card">
          ...
          <div className="px-3 py-2 border-t text-xs flex justify-between">
            ...
            <span className="font-semibold">{formatDZD(breakdown.manufacturingSaleTotal)}</span>
          </div>
        </section>

        {/* SYNTHÈSE */}
        <section className="rounded-lg border bg-muted/30 p-3 space-y-2 text-sm">
          <div className="flex justify-between"><span className="text-muted-foreground">التكلفة الإجمالية للطلبية</span><span className="font-semibold">{formatDZD(breakdown.totalCost)}</span></div>
          <div className="flex justify-between"><span className="text-muted-foreground">تكلفة الوحدة ({order.quantity})</span><span className="font-semibold">{formatDZD(breakdown.unitCost)}</span></div>
          <div className="flex items-center justify-between gap-3 pt-1">
            <Label className="text-muted-foreground font-normal">ثمن بيع الوحدة</Label>
            <div className="w-48"><MoneyInput value={draftSalePrice} onValueChange={setDraftSalePrice} /></div>
          </div>
          <div className="flex justify-between border-t pt-2">
            <span className="text-muted-foreground">ثمن البيع الإجمالي</span>
            <span className="font-bold text-base">{breakdown.totalSalePrice != null ? formatDZD(breakdown.totalSalePrice) : '—'}</span>
          </div>
        </section>
      </fieldset>
