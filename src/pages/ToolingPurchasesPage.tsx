...
const ToolingPurchasesPage: React.FC = () => {
  ...
  const handleExportExcel = () => {
    exportTableToExcel('مشتريات العدة', filteredRows.map((r: any) => ({
      ...
      'العدة المطلوبة': r.items && r.items.length > 0 ? r.items.map((i: ResourceItem) => `${i.label} (${i.status === 'non-disponible' ? 'non disponible' : 'partiellement disponible'})`).join('\n') : '—',
        ...
        <table className="w-full caption-bottom text-sm">
          <TableHeader>
            <TableRow>
              ...
              <TableHead>العدة المطلوبة</TableHead>
